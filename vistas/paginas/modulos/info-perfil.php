...
$root   = dirname(__DIR__, 1); // por ejemplo, si estás en /vistas/ y config está en la raíz
$config = require $root . '/config/config.php';

// Public key de Mercado Pago desde config
...
echo "\n<!-- SESSION_ID=".(int)($_SESSION['id'] ?? 0)." -->\n";
foreach (['codigoReserva','reservaUid','idHabitacion','fechaIngreso','fechaSalida'] as $c) {
  $v = isset($_COOKIE[$c]) ? $_COOKIE[$c] : 'NO';
  echo "<!-- COOKIE_$c=$v -->\n";
}
?>
 <?php if ( !isset($_GET['ok']) && isset($_COOKIE['codigoReserva']) && ( !isset($_COOKIE['reservaUid']) || (int)$_COOKIE['reservaUid'] === (int)($_SESSION['id'] ?? 0) ) ): ?> 
...
$silenciarMensajesPerfil = true;
  $ing = $_COOKIE['fechaIngreso'] ?? null;
  $sal = $_COOKIE['fechaSalida']  ?? null;

  if ($ing && $sal) {

    // Validaciones básicas de fecha (formato Y-m-d soporta comparación lexicográfica)
    $hoy = date('Y-m-d');
    if ($ing <= $hoy || $sal <= $hoy || $sal <= $ing) {
     if (!$silenciarMensajesPerfil) {
    echo '<div class="alert alert-danger">Fechas inválidas. Verifica ingreso y salida.</div>';
  }  $validarPagoReserva = false;
    } else {

      // En principio OK, salvo que encontremos cruce
      $validarPagoReserva = true;

      // CRUCE DE FECHAS con reservas existentes de la misma habitación
      $idHab = $_COOKIE['idHabitacion'] ?? null;
      if ($idHab) {
        $reservasExist = ControladorReservas::ctrMostrarReservas($idHab);

        if (!empty($reservasExist)) {
          foreach ($reservasExist as $res) {
            $rIng = $res['fecha_ingreso']; // Y-m-d
            $rSal = $res['fecha_salida'];  // Y-m-d

            // Intersección de intervalos semiabiertos: [ing, sal) ∩ [rIng, rSal)
            $cruce = ($ing < $rSal) && ($sal > $rIng);
...
<!-- Contenedor del Brick (DEBE existir antes del script) -->
<div id="paymentBrick_container" class="w-100 pt-3"></div>
...
(function () {
  ...
  // Clave pública de pruebas (Perú)
  const mp = new MercadoPago("<?= htmlspecialchars($mpPublicKey, ENT_QUOTES, 'UTF-8') ?>", { 
    locale: "es-PE" 
  });
  ...
  const settings = {
    initialization: {
      // El Brick exige amount, pero tu backend calculará el monto final igualmente
      amount: <?= (int)($_COOKIE["pagoReserva"] ?? 1) ?>,
      payer: { email: "<?= htmlspecialchars($usuario['email']) ?>",entityType: "individual" }
    },
    customization: {
    ...
    callbacks: {
      onReady: () => {
        // aquí podrías ocultar un spinner si tenías uno
      },
      onSubmit: ({ selectedPaymentMethod, formData }) => {
        const reserva = {
          idHabitacion:  "<?= $_COOKIE['idHabitacion']   ?? '' ?>",
          fechaIngreso:  "<?= $_COOKIE['fechaIngreso']   ?? '' ?>",
          fechaSalida:   "<?= $_COOKIE['fechaSalida']    ?? '' ?>",
          plan:          "<?= $_COOKIE['plan']           ?? 'Plan Continental' ?>",
          personas:      2,
          codigoReserva: "<?= $_COOKIE['codigoReserva']  ?? '' ?>",
          infoHabitacion:"<?= $_COOKIE['infoHabitacion'] ?? '' ?>",
          payerEmail:    "<?= $usuario['email'] ?>"
        };

        // IMPORTANTE: devolver una Promesa y llamar resolve()/reject()
       return new Promise((resolve, reject) => {
  fetch("<?= $ruta ?>ajax/pagos.procesar.php", {
    method: "POST",
    headers: { "Content-Type": "application/json", "Accept": "application/json" },
    credentials: "same-origin",
    body: JSON.stringify({ cardFormData: formData, reserva })
  })
  .then(async (r) => {
    const text = await r.text(); // lee siempre como texto
    let data;
  ...
  .then((data) => {
    if (data.status === "approved" && data.saved) {
      window.location = "<?= $ruta ?>perfil?ok=1";
    } else {
      console.error("Respuesta MP:", data);
      alert("Estado: " + (data.status || data.message || "No se pudo procesar el pago"));
    }
    resolve();
  })
  .catch((err) => {
    console.error(err);
    alert(err.message || "Error de red al procesar el pago.");
    reject();
  });
});
      ...
      onError: (error) => {
        // requerido por el Brick; loguea cualquier error de inicialización/uso
        console.error("Brick error:", error);
        alert("Ocurrió un error inicializando el pago.");
      }
    }
  };

  bricksBuilder.create("payment", containerId, settings);
})();
</script>
